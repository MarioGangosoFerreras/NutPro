...
import {
  ...
  IonSegment, IonSegmentButton, IonToggle, IonCard, IonCardContent,
  IonCardHeader, IonCardTitle, IonModal,
  ToastController, AlertController
} from '@ionic/angular/standalone';
import { addIcons } from 'ionicons';
import {
  arrowBackOutline, personCircleOutline, calendarOutline, callOutline,
  mailOutline, locationOutline, createOutline, closeOutline, saveOutline,
  cameraOutline, trashOutline, cardOutline, briefcaseOutline, heartOutline,
  flagOutline, addOutline, scaleOutline, bodyOutline, medkitOutline,
  restaurantOutline, checkmarkOutline, closeCircleOutline
} from 'ionicons/icons';

@Component({
  selector: 'app-ficha-paciente',
  imports: [
    RouterLink, Header, FormsModule,
    IonContent, IonButton, IonIcon, IonSpinner, IonBadge, IonAvatar,
    IonItem, IonLabel, IonInput, IonSelect, IonSelectOption, IonTextarea,
    IonSegment, IonSegmentButton, IonToggle, IonCard, IonCardContent,
    IonCardHeader, IonCardTitle, IonModal
...
export class FichaPaciente implements OnInit {
  ...
  async ngOnInit() {
    ...
    if (!id) { this.router.navigate(['/pacientes']); return; }
    try {
  ...
  private async cargarDatosClinica() {
    ...
    try {
      ...
      this.formAntFam = fam ? { ...fam } : {
        hta: false, ecv: false, diabetes: false, enfermedad_autoinmune: false,
        alergias: false, obesidad: false, cancer: false, otra: ''
      };
      this.formAntPer = per ? { ...per } : {
        alcohol: false, tabaco: false, cirugias: false, alergias: false,
        enfermedad_patologia: '', medicacion: '', digestiones: '',
        menstruacion: '', suplementos_nutricionales: '',
        descansa_bien: false, actividad_fisica: null
      };
    } catch (e) { console.error(e); }
    finally { this.loadingClinica = false; this.cdr.detectChanges(); }
  }
  ...
  private async cargarEncuesta() {
    ...
    try {
      ...
      this.formEncuesta = enc ? { ...enc } : {
        ...
        consumo_agua_litros: null, otras_bebidas: '',
        preferencias_alimentarias: '', aversiones_alimentarias: ''
      };
    } catch (e) { console.error(e); }
    finally { this.loadingClinica = false; this.cdr.detectChanges(); }
  }
  ...
  async confirmarEliminarMedicion(id: string) {
    const alert = await this.alertCtrl.create({
      ...
      buttons: [
        ...
        {
          text: 'Eliminar', role: 'destructive', cssClass: 'alert-btn-danger',
          handler: async () => {
  ...
  private resetNuevaMedicion() {
    this.nuevaMedicion = {
      ...
      masa_muscular_kg: null, perimetro_cintura_cm: null, notas: ''
    };
  }
  ...
  async guardar() {
    ...
    try {
      if (this.avatarFile) {
        ...
        else { await this.mostrarToast('Error al subir la imagen', 'danger'); return; }
      }
  ...
  async confirmarEliminar() {
    const alert = await this.alertCtrl.create({
      ...
      buttons: [
        { text: 'Cancelar', role: 'cancel' },
        { text: 'Eliminar', role: 'destructive', cssClass: 'alert-btn-danger', handler: () => this.eliminar() }
      ]
    });
    await alert.present();
  }
